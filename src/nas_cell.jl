...
\text{Second Layer Computations:} & \quad
l_1 &= \tanh(o_1 \cdot o_2) \quad \text{(combining outputs 1 and 2)}, \\
l_2 &= \tanh(o_3 + o_4) \quad \text{(combining outputs 3 and 4)}, \\
l_3 &= \tanh(o_5 \cdot o_6) \quad \text{(combining outputs 5 and 6)}, \\
l_4 &= \sigma(o_7 + o_8) \quad \text{(combining outputs 7 and 8)}.

\text{Inject Cell State:} & \quad
l_1 &= \tanh(l_1 + c_{\text{state}}) \quad \text{(adding the cell state to the first output)}.

\text{Final Layer Computations:} & \quad
c_{\text{new}} &= l_1 \cdot l_2 \quad \text{(new cell state)}, \\
l_5 &= \tanh(l_3 + l_4) \quad \text{(combining second-layer outputs)}, \\
h_{\text{new}} &= \tanh(c_{\text{new}} \cdot l_5) \quad \text{(new hidden state)}.
\end{aligned}
```
